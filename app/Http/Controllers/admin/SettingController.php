...
class SettingController extends Controller
{
   ...
   public function index(Request $request)
   {
      ...
      'logo2' => 'mimes:jpeg,bmp,png,jpg|max:2000',
          ...
          foreach($_POST as $key => $value){
		   ...
		   if($request->hasFile('logo')) {
            ...
            if($request->hasFile('logo2')) {
                $storagepath = $request->file('logo2')->store('public/logo');
                $fileName = basename($storagepath);
                $logo2['name']='logo2';
                $logo2['value'] = $fileName;

                //if file chnage then delete old one
                $oldFile = $request->get('oldLogo2','');
                if( $oldFile != ''){
                    $file_path = "public/logo/".$oldFile;
                    Storage::delete($file_path);
                }
              ...
              $logo2['name']='logo2';
                $logo2['value'] = $request->get('oldLogo2','');
            }
      ...
      if(Setting::where('name', "logo2")->exists()){       
        Setting::where('name','=',"logo2")->update($logo2);     
      }else{
         
        $logo2['created_at'] = Carbon::now();
        Setting::insert($logo2); 
      }
